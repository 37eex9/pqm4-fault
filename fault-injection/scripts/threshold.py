...
# This file was *autogenerated* from the file threshold.sage
from sage.all_cmdline import *   # import sage library

_sage_const_2 = Integer(2); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_4 = Integer(4); _sage_const_3 = Integer(3)#!/usr/bin/env sage
...
class Calc_Threshold:
    ...
    def __init__(self, r, d, t):
        ...
        self.t = t
        self.d = d
        self.w = _sage_const_2 *d
        self.n = _sage_const_2 *r

        self.max_s = None  # maximaler wert von s bevor th(s) komplexwertig wird

        self.rho = [_sage_const_0 ] * (_sage_const_2  * self.w + _sage_const_2 )
        for l in range(_sage_const_0 , _sage_const_2  * self.w + _sage_const_2 ):
        ...
        self.B = log(self.t / (self.n - self.t), _sage_const_2 )
    ...
    def _x_coeff(self):
        ...
        """
        buff_a = _sage_const_0 
        buff_b = _sage_const_0 
        for l in range(_sage_const_0 , self.w):
            buff_a = buff_a + _sage_const_2 *l*self.rho[_sage_const_2 *l+_sage_const_1 ]
            buff_b = buff_b + self.rho[_sage_const_2 *l+_sage_const_1 ]
    ...
    def calc_pi0(self, s, X):
        """compare to Section 5.3 Equation 5.1 in master thesis Fault attacks on BIKE
        """
        pi_0 = ((self.w-_sage_const_1 )*s- X) / self.pi0_coeff
        return pi_0
    ...
    def calc_T(self, s):
        ...
        if pi_1 > _sage_const_1 :
            T = self.d
            if self.max_s is None:
                self.max_s = s - _sage_const_1 
            return T, True
        else:
            A = self.d * log((_sage_const_1  - pi_1) / (_sage_const_1  - pi_0), _sage_const_2 )
            #B = log(e / (self.n - e), 2)
            C = log((_sage_const_1  - pi_1) / (_sage_const_1  - pi_0), _sage_const_2 )
            D = log(pi_0/pi_1, _sage_const_2 )
            T = ((A + self.B) / (C + D)).n() # zusammenfügen
            T = max(int(T) + _sage_const_1 , _sage_const_0 )#, (self.d + 1)/2)  # aufrunden und die untere Grenze setzen
            return T, False
    ...
    def s_boundarys(self):
        ...
        """
        l_bound = _sage_const_1 
        u_bound = self.r-_sage_const_1 
    ...
    def compare(self, show=False):
        ...
        for s in range(l_bound, u_bound):
            ...
            if done:
                for i in range(s+_sage_const_1 , u_bound):
                    thresholds.append(th)
                break
        ...
        if self.max_s:
            a, b = var('a', 'b')
            eq1 = a + _sage_const_1  * b == thresholds[_sage_const_0 ]  # Ergebnisse im Array beginnen bei s = 1 nicht 0, desshalb muss ich 1 abziehen
            eq2 = a + self.max_s * b == thresholds[self.max_s - _sage_const_1 ]
            sol = solve([eq1, eq2], a, b, solution_dict=True)
            res_a = sol[_sage_const_0 ][a]
            res_b = sol[_sage_const_0 ][b]
            for s in range(l_bound, u_bound):
