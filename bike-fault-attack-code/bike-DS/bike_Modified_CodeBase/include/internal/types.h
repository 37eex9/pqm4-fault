...
// add struct for meta information

typedef void (*Insert_Custom_Error_Function)(idx_t[D], idx_t[D], uint8_t[R_BYTES], uint8_t[R_BYTES], int, int);

typedef void (*Change_Error_Vectors_Fun)(uint8_t[R_BYTES], uint8_t[R_BYTES]);

// struct to track the errors per round
struct Meta_Info{
    // number of errors after round i
    int rounds[6];
    int succ;
};

// struct to track the errors /ciphertest used
struct Meta_Info_enc{
    unsigned char e0[R_BYTES];
    unsigned char e1[R_BYTES];
    unsigned char c0[R_BYTES];
    int succ;
};
