...
int generateRandomNumber(int n) {
    ...
    while (rand() / (double)RAND_MAX < probability && result < n) {
        probability /= 1.00005;
        result++;
    }
...
// to overwrite by function call 
// defined in header file
// typedef void (*Insert_Custom_Error_Function)(idx_t[D], idx_t[D], uint8_t[R_BYTES], uint8_t[R_BYTES], int);
                 ...
                 OUT unsigned char *sk)
{
 ...
 error_pos_in_h1[0] = 0;
  ...
  // CHANGED to insert the error 
  if(custom_error_function != NULL){
    custom_error_function(l_sk.wlist[0].val, l_sk.wlist[1].val, h0.val.raw, h1.val.raw, error_h0, error_h1);
  }
  else{
    printf("no custom error function provided \n");
  }
  
  /*
  printf("h before : ");
  for (int x = 0; x < R_BYTES; x++){
    printf("%02x ", h0.val.raw[x]);
  }
  printf("\n");
  */
  ...
  // to see
  DEFER_CLEANUP(pad_r_t ht = {0}, pad_r_cleanup);
  gf2x_mod_mul(&ht, &h0, &h0inv);


  /*
  printf("h inv : ");
  for (int x = 0; x < R_BYTES; x++){
    printf("%02x ", h0inv.val.raw[x]);
  }
  printf("\n");
  */
  ...
  print("sk : ", sizeof(l_sk),sizeof(sizeof(l_sk)) );
...
int change_ei(OUT unsigned char* e_0, OUT unsigned char* e_1){
  ...
  for(int i = 0; i < R_BYTES; i++){
    counted_in_e1 += __builtin_popcount(e_1[i]);
    e_1[i] = 0;
  }

  int pos_to_fix = rand() % R_BITS;

  int wanted_fixed_pos = 10;
  int num_fixed_pos = min(wanted_fixed_pos,counted_in_e1);
  for (int i = 0; i < num_fixed_pos; i++)
  {
    set_bit_in_e(e_1, (pos_to_fix +i) % R_BITS);
    counted_in_e1--;
  }
  
  while (counted_in_e1 > 0){
    int pos = rand() % R_BITS;
    if(set_bit_in_e(e_1 , pos)) counted_in_e1--;
  }
                   ...
                   //IN int change_e)
{
  ...
  if(change_error_vectors_fun != NULL){
    change_error_vectors_fun(e.val[0].val.raw, e.val[1].val.raw);
  }


  // save e to compare
  for (int i = 0; i < R_BYTES; i++){
    e_out[0][i] = (unsigned char)e.val[0].val.raw[i];
    e_out[1][i] = (unsigned char)e.val[1].val.raw[i];
  }
                   ...
                   IN const unsigned char *sk)
{
  ...
  // Decode and check if success.
  int success_cond = (decode(&e, &l_ct, &l_sk) == SUCCESS);
  
  // save to verify success or failure
  for (int i = 0; i < R_BYTES; i++){
    e_out[0][i] = (unsigned char)e.val[0].raw[i];
    e_out[1][i] = (unsigned char)e.val[1].raw[i];
  }
  

  return SUCCESS; 
  // early return cause we only wanna see if e is correct
