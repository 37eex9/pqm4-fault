...
ret_t decode(OUT e_t *e, IN const ct_t *ct, IN const sk_t *sk)
{
  ...
  gf2x_mod_mul(&ht, &c0, &h0);
  printf("s normal : \n");
  for (int x = 0; x < R_BYTES; x++){
    printf("%02x ", ht.val.raw[x]);
  }
  printf("\n");

  DEFER_CLEANUP(pad_r_t ht_inv = {0}, pad_r_cleanup);
  for (int x = 0; x < R_BYTES; x++)
  {
    ht_inv.val.raw[x] = 0xff;
  }
  ht_inv.val.raw[0] = 0xfe;
  ht_inv.val.raw[R_BYTES-1] = 0x07;
  
  gf2x_mod_mul(&ht, &h0, &ht_inv);
  printf("s normal h*h^-1 : \n");
  for (int x = 0; x < R_BYTES; x++){
    printf("%02x ", ht.val.raw[x]);
  }
  printf("\n");
  */
  ...
  uint32_t MAX_It_tmp = MAX_IT;//2;
  for(uint32_t iter = 0; iter < MAX_It_tmp; iter++) {
...
ret_t decode_meta(OUT e_t *e, OUT struct Meta_Info *info, IN const ct_t *ct, IN const sk_t *sk)
{
  ...
  for(uint32_t iter = 0; iter < MAX_IT; iter++) {
    ...
    DMSG("    Weight of e: %lu\n",
         r_bits_vector_weight(&e->val[0]) + r_bits_vector_weight(&e->val[1]));
    DMSG("    Weight of syndrome: %lu\n", r_bits_vector_weight((r_t *)s.qw));
    //printf("    Weight of syndrome: %lu\n", r_bits_vector_weight((r_t *)s.qw));

    // save the round information
    info->rounds[iter] = r_bits_vector_weight((r_t *)s.qw);
  ...
  // safe the last round
  info->rounds[MAX_IT] = r_bits_vector_weight((r_t *)s.qw);
  //info->rounds[0] = 9;
