...
// trigger pins
#define TRIG_REG GPIOA
#define TRIG_PIN GPIO12
#define TRIG_RCC RCC_GPIOA
#define STM32
#define CW_BOARD
...
{
  uint8_t d  = c;
  usart_send_blocking(SERIAL_USART, d);
}

void hal_receive(uint8_t * recv, uint16_t length)
{
  uint16_t len = length;
  uint8_t* ptr = recv;
  while (len) {
...
void hal_send(uint8_t * send, uint16_t length)
{
  uint16_t len = length;
  uint8_t* ptr = send, len_l, len_h;
...
// cw api
# ifdef TRIGGER_ADV
size_t trigger_enable = 0, trigger_h0 = 0, trigger_h1 = 0;
...
{
  trigger_enable = trigger_h0;
}

void trigger_enable_h1(void)
{
  trigger_enable = trigger_h1;
}
...
{
  gpio_set(TRIG_REG, TRIG_PIN);
}

void trigger_low(void)
{
  gpio_clear(TRIG_REG, TRIG_PIN);
}

void trigger_setup(void)
{
  rcc_periph_clock_enable(TRIG_RCC);
  gpio_mode_setup(TRIG_REG, GPIO_MODE_OUTPUT,GPIO_PUPD_NONE ,TRIG_PIN);
  gpio_set_output_options(TRIG_REG, GPIO_OTYPE_PP, GPIO_OSPEED_100MHZ, TRIG_PIN);
  trigger_low();
}
#endif // TRIGGER
