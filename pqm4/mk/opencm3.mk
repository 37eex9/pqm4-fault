LIBHAL_SRC := \
	...
	mupq/pqclean/test/common/nistkatrng.c

obj/libpqm4hal.a: $(call objs,$(filter-out mupq/pqclean/test/common/nistkatrng.c,$(LIBHAL_SRC)))
obj/libpqm4hal-nornd.a: $(call objs,$(filter-out mupq/pqclean/test/common/nistkatrng.c, $(filter-out common/randombytes.c,$(LIBHAL_SRC))))
obj/libpqm4hal-kat.a: $(call objs,$(filter-out common/randombytes.c,$(LIBHAL_SRC)))

ifeq ($(AIO),1)
LDLIBS +=
LIBDEPS += $(if $(NO_RANDOMBYTES),$(filter-out mupq/pqclean/test/common/nistkatrng.c, $(filter-out common/randombytes.c,$(LIBHAL_SRC))),\
			$(if $(NIST_KAT_RANDOMBYTES), $(filter-out common/randombytes.c,$(LIBHAL_SRC)),\
			$(filter-out mupq/pqclean/test/common/nistkatrng.c,$(LIBHAL_SRC))))
else
LDLIBS += -lpqm4hal$(if $(NO_RANDOMBYTES),-nornd)$(if $(NIST_KAT_RANDOMBYTES),-kat)
LIBDEPS += obj/libpqm4hal$(if $(NO_RANDOMBYTES),-nornd)$(if $(NIST_KAT_RANDOMBYTES),-kat).a
endif
